Durch die Berechnung der Testabdeckung will der Tester erfahren, wie viel von dem Prüfling beim Testen ausgeführt wurde. Für die Definition des Prüflings, in diesem Fall des BPEL-Prozesses, wird die BPEL Formalisierung von Ouyang in \cite{Chun2005} verwendet. Nach der Präsentation der wichtigsten Punkte dieser Formalisierung folgt die Definition der  Aktivitäts- und Zweigabdeckung. Darüber hinaus werden zwei speziell für BPEL definierte Metriken vorgestellt: Link- und Handler-Abdeckung.  
\section{Formale Definition von BPEL}\label{sec:bpelformal}
Die hier vorgestellte formale Beschreibung des BPEL-Prozessmodels ist nicht
vollständig und erfasst nicht den gesamten Umfang von BPEL. Es werden nur
...
Standard BPEL 2.0 und einigen Erweiterungen, unverändert übernommen wurde.  

Als Erstes wird eine Auswertungsfunktion definiert. Sei $f$ eine boolsche Funktion (oder ein boolscher Ausdruck),
$Var(f)$ enthält aussagenlogische Variablen, die in $f$ vorkommen. Sei $F$ die Menge
der boolschen Funktionen und $B$ die Menge (\textit{true}, \textit{false}), dann ist die Variablenzuordnung von $F$ die Abbildung $assign: Var(F)\rightarrow B$, und eine Menge aller möglichen Variablenzuordnungen von $F$ ist $Assign(F)$. Die Auswertungsfunktion ist $eval: F \times Assign(F)\rightarrow B$.
...
\end{itemize}
 $last(s)$ repräsentiert den \textit{else}-Zweig in \textit{if}-Aktivität, was zum einem,  sichert dass ein \textit{else}-Zweig immer vorhanden ist und zum anderen mindestens ein Zweig in der Aktivität aktiviert wird,
...
\end{itemize}
\item $process\in \mathcal{A}_{scope}$ ist die Wurzel des $\mathcal{HR}$ Baumes ,
