...
Für die Messung der Testabdeckung mit dem Instrumentierungsansatz müssen BPEL-Dateien vor der Ausführung modifiziert werden. Dafür müssen die Dateien erst aus dem Archive extrahiert und nach der Instrumentierung wieder in den Archiv geschrieben werden. Außerdem muss Archive weitere Informationen für Engine bereithalten: die Beschreibung der Schnittstelle des Web Services zum Empfangen von Marken (\textit{Coverage Logging Service}) und wo dieser Service erreicht werden kann.
\begin{itemize}
...
BPELUnit-Framework wird durch Hinzufügen einer Phase vor dem tatsächlichen Testen. In dieser Phase wird der BPEL-Prozess annotiert, \textit{invoke}-Aktivitäten werden hinzugefügt erweitert. Diese Aktivitäten sind Markierungen für jede Basisaktivität und jede Kontrollflussmöglichkeit. 
...
BPELUnit-Framework wurde mit einer Komponente erweitert, die alle Aufgaben, die mit der Messung der Abdeckung zu tun haben, verwaltet (Abbildung \ref{fig:BPELUnitErweiterung}). So enthält sie einen Instrumentierer der die BPEL-Datei analysiert und Annotationen hinzufügt. Die Originaldatei wird durch annotierte Kopie ersetzt.  Zusätzlich ist ein Web Service für das Empfangen von Marken (\textit{Coverage Logging Service}) in dieser Komponente enthalten. 

Das Framework ersetzt die Kommunikationspartner des BPEL-Prozesses durch Mock's, die die Geschäftsprozesse des Partners simulieren. Auf dieselbe Weise wird das Empfangen von Marken realisiert. Der \textit{Coverage Logging Service} wird durch Testumgebung zur Verfügung gestellt (Abbildung \ref{fig:loggingservice}). 
 \begin{figure}[htbp]
	\centering
...
\subsection{Web Service zum Empfangen von Marken (\textit{Coverage Logging Service})}
Das BPELUnit-Framework simuliert die Partner des Prozesses auf einem lokalen Http-Server, der im Framework integriert ist.  Zum Einsatz kommt open-source Webserver \textit{Jetty} in der Version 5.1. Bei der Ausführung einer Testsuite wird der Webserver gestartet und ein Handler eingerichtet, der die Kommunikation zwischen dem Prozess und seinen Partnern regelt.   

Der Port, auf dem die Partner simuliert werden, wird von dem Testenwickler in den Testsuites festgelegt. Der \textit{Coverage Logging Service} wartet auf Nachrichten auf dem Port 7778, der für ihn reserviert ist. Der Empfang und Auswertung der Marken wird von einem speziellen Handler unabhängig von allen anderen Nachrichten ausgeführt.
...
Dabei gibt es ein Problem: es kann vorkommen, dass während die (\textit{Coverage-})Nachrichten eines Testfalls noch übertragen werden, bereits alle Geschäftsinteraktionen des Testfalls abgearbeitet wurden und ein neuer Testfall gestartet wird. In diesem Fall findet falsche Zuordnung der Marken an die Testfälle statt. Beim letzten Testfall gehen die Marken sogar verloren, weil der Webserver, auf dem der \textit{Coverage Logging Service} läuft, heruntergefahren wird. Das Problem wird behoben, indem nach jedem Testfall, nach Beenden aller Interaktionen des Testfalls, eine bestimmte Zeitspanne vor dem Starten des nächsten Testfalls bzw. Herunterfahren des Webservers abgewartet wird. Dadurch wird zwar die Zeit, die ein Test in Anspruch nimmt vergrößert, dafür liefert die Testabdeckung korrekte Werte. Die \textit{default}-Wartezeit ist auf  500ms gesetzt kann aber in der Konfigurationsdatei oder bei Eclipse-Plugin über die Konfigurationsmenü angepasst werden. Das Sequenzdiagramm in der Abbildung \ref{fig:sequenzediagrammTestcase} stellt die entsprechenden Interaktionen grafisch dar. 
 ...
 \begin{figure}[htbp]
	\centering
		\includegraphics[width=0.8\textwidth]{bilder/Sequenzdiagram.pdf}
