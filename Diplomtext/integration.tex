...
BPELUnit-Framework wird durch Hinzufügen einer Phase vor dem tatsächlichen Testen. In dieser Phase wird der BPEL-Prozess annotiert , invoke-Aktivitäten werden hinzugefügt erweitert. Diese Aktivitäten sind Markierungen für jede Basisaktivität und jede Kontrollflussmöglichkeit. 
Zur Laufzeit rufen die Aktivitäten ein \textit{Coverage Logging Service} aus. Bei diesen Aufrufen werden die Marken transportiert, die den Ausführungspfad zum BPELUnit-Framework bzw. der Komponente für die Messung der Testabdeckung signalisieren. Nach der Ausführung können die Abdeckungsmetriken  anhand der empfangenen Marken und  statistischen Informationen über BPEL-Prozess  berechnet werden. 
...
Das Framework ersetzt die Kommunikationspartner des BPEL-Prozesses durch Mock's, die die Geschäftsprozesse des Partners simulieren. Auf dieselbe Weise kann das Empfangen von Marken realisiert werden. Der \textit{Coverage Logging Service} wird durch Testumgebung zur Verfügung gestellt (Abbildung \ref{fig:loggingservice}). 
 \begin{figure}[htbp]
	\centering
...
\end{verbatim}
Soll eine Metrik nicht berechnet werden, so kann sie einfach auskommentiert werden. Für die Aktivitätabdeckung können zusätzlich die Basisaktivitäten festgelegt werden, die berücksichtigt werden sollen. Mit dem Parameter \textit{waitTime} kann die Dauer festgelegt werden, die nach jedem Testfall abgewartet wird, um alle (\textit{Coverage-})Nachrichten zu empfangen. Beim fehlen des Attributs wird der Wert auf 500 ms gesetzt. Zu beachten ist, dass kleine Werte zwar insgesamt die Verzögerung, die aufgrund der Testabdeckungsmessung zustande kommt, aber zur falschen Testabdeckung führen können.
...
\begin{figure}[htbp]
	\centering
		...
		\caption{Ergebnisdarstellung in Eclipse}
