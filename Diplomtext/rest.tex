...
WSDL (siehe nächsten Abschnitt) beschrieben.
...
Unit-Tests spielen eine sehr wichtige Rolle bei der Qualitätssicherung in der Softwareentwicklung. 
...
Für die Messung heißt es, während es bei normalen Kanten ausreichend ist, die aktivierten Kanten zu erfassen, muss es bei den Links zusätzlich zwischen \textit{true}- und \textit{false}-Status unterschieden werden. 
...
Es wird eine Umsetzung der  Die Tests, die
auf der Codeabdeckung basieren, geh¨oren zur Gruppe der strukturorientierten
Testtechniken und zielen darauf ab, die innere Struktur der Software zu testen 

Während für das Testen, als ein Mittel zur Qualitätssicherung, schon einige Konzepte und Lösungen für  BPEL-Kompositionen  erarbeitet wurden, fehlt es an Testabdeckungsmetriken.



  

BPEL ist Standard und wird von vielen unterstützt. Sichern die Qualität wird kritisch.  Eine Möglichkeit Sofware qualität zu sichern ist das Testen in vielen formen darunter  als Codezentrierter Ansatz wiederholbare white box tests. 

In anderen Regionen ...

Der Aufstieg von SOA und Web Service Stack von STandards hat viele Unternehemen zur Bildung neuen flexiblen Architekturen bewegt. Services wurden entwickelt, die in so genannten Kompositionen mit dem Ziel realisierung voll automatischen Geschäftsprozessen 


BPEl BPEL ist standardisiert von OASIS und wird unterstütz  Vor der Nutzung  von BPEL -Prozessen , diese Systemen müssen intensive getestet werden  um das Risiko zu minimieren bei den Fehler angesichts der wichtigen Rolle in dem Unternehmen. 

Aber Web Services und BPEL sind eine relativ neue Technologiefeld und viele Methoden, die bei den Software entwicklung zur Verfügung stehen wurden aus SOA noch nicht übertragen oder SOA-Erfahrungen fehlen. In diseser Arbeit werden Testabdeckungsmetriken formal definiert. Testabdeckung ist Metrik wie viele von software ausgeführt wurde. Diese werden oft benutzt um die Qualität der Tests abzuleiten. Außerdem wird ein BPELUNit erwitert.  
...
Das Hauptziel von BPELUnit Framework ist unit Test, die natürliche Benutzergruppe sind entwickler.
...
xmlns:ns1="http://order.service.coverage.example.se.uni.hannover.de" 
xmlns:ns2="http://auth.service.coverage.example.se.uni.hannover.de" 
xmlns:ns3="http://message.service.coverage.example.se.uni.hannover.de" 
xmlns:xsd="http://www.w3.org/2001/XMLSchema" name="prozess" suppressJoinFailure="yes" targetNamespace="http://prozess">
   <bpel:import importType="http://schemas.xmlsoap.org/wsdl/" 
   location="../wsdl/OrderService.wsdl" 
   namespace="http://order.service.coverage.example.se.uni.hannover.de"/>
   <bpel:import importType="http://schemas.xmlsoap.org/wsdl/" 
   location="../wsdl/authentifikationService.wsdl" 
   namespace="http://auth.service.coverage.example.se.uni.hannover.de"/>
   <bpel:import importType="http://schemas.xmlsoap.org/wsdl/" location="../wsdl/MessagingService.wsdl"
    namespace="http://message.service.coverage.example.se.uni.hannover.de"/>
   <bpel:partnerLinks>
      <bpel:partnerLink myRole="orderService" name="PLTOrderService" partnerLinkType="ns1:PLTOrderService"/>
      <bpel:partnerLink name="PLTAuthenticateService" partnerLinkType="ns1:PLTAuthenticateService" partnerRole="AuthenticateService"/>
      <bpel:partnerLink name="PLTMessageService" partnerLinkType="ns1:PLTMessageService" partnerRole="messageService"/>
   </bpel:partnerLinks>
   <bpel:variables>
      <bpel:variable messageType="ns1:receiveOrderRequest" name="orderRequest"/>
      <bpel:variable messageType="ns1:receiveOrderResponse" name="orderResponse"/>
      <bpel:variable messageType="ns2:authenticateRequest" name="authenticateRequest"/>
      <bpel:variable messageType="ns2:authenticateResponse" name="authenticateResponse"/>
      <bpel:variable messageType="ns3:notifyRequest" name="notifyRequest"/>
   </bpel:variables>
   <bpel:faultHandlers>
      <bpel:catchAll>
         <bpel:sequence>
            <bpel:assign>
               <bpel:copy>
                  ...
                  <bpel:to part="in" variable="notifyRequest"/>
               </bpel:copy>
            </bpel:assign>
            <bpel:invoke inputVariable="notifyRequest" operation="notify" partnerLink="PLTMessageService" portType="ns3:MessageService"/>
         </bpel:sequence>
      </bpel:catchAll>
   </bpel:faultHandlers>
   <bpel:sequence>
      <bpel:receive createInstance="yes" operation="receiveOrder" partnerLink="PLTOrderService" portType="ns1:OrderHandler" 
                                                                                                                    variable="orderRequest"/>
      <bpel:if>
         <bpel:condition expressionLanguage="urn:oasis:names:tc:wsbpel:2.0:sublang:xpath1.0">$orderRequest.in/shipment='express'</bpel:condition>
         <bpel:assign>
            <bpel:copy>
               <bpel:from>
                  <bpel:literal>Hallo</bpel:literal>
               </bpel:from>
               <bpel:to part="id" variable="authenticateRequest"/>
            </bpel:copy>
            <bpel:copy>
               <bpel:from part="in" variable="orderRequest">
                  <bpel:query>customerId</bpel:query>
               </bpel:from>
               <bpel:to part="id" variable="authenticateRequest"/>
            </bpel:copy>
         </bpel:assign>
         ...
         <bpel:sequence>
            <bpel:sources>
               <bpel:source linkName="L1">
                  ...
                  $authenticateResponse.out/customerType='VIP'</bpel:transitionCondition>
               </bpel:source>
            </bpel:sources>
            <bpel:invoke inputVariable="authenticateRequest" operation="authenticate" outputVariable="authenticateResponse" 
                                                             partnerLink="PLTAuthenticateService" portType="ns2:AuthenticationService"/>
            <bpel:assign>
               <bpel:copy>
                  <bpel:from>
                     <bpel:literal>Order is accepted</bpel:literal>
                  </bpel:from>
                  <bpel:to part="message" variable="orderResponse"/>
               </bpel:copy>
            </bpel:assign>
         </bpel:sequence>
      </bpel:flow>
      <bpel:reply operation="receiveOrder" partnerLink="PLTOrderService" portType="ns1:OrderHandler" variable="orderResponse"/>
   </bpel:sequence>
</bpel:process>
