...
Für die korrekte Ermittlung der Testabdeckung ist die richtige Platzierung der Marken entscheidend. Dank der Zweiteilung der Instrumentierungsphase kann dieses Problem behandelt werden, ohne die BPEL-spezifischen Aspekte der Kommunikation berücksichtigen zu müssen.
...
In dieser Phase erfolgt die Annotation des Prozesses mit speziellen Marken. Jede Marke ist eindeutig identifizierbar und gehört zu einer bestimmten Metrik. Außerdem identifiziert jede Marke ein Element des Kontrollflusses, das für die Abdeckungsmessung relevant ist. Liegen die Marken während des Tests auf dem Ausführungspfad, so werden diese an eine Komponente übertragen. Damit protokolliert jede Marke Stimulation eines bestimmten Kontrollflusselements. Obwohl die Realisierung der Datenübertragung erst im zweiten Schritt erfolgt (siehe Abschnitt \ref{sec:markenuebetragung}), wird zur Vereinfachung bereits in diesem Abschnitt statt einer Marke auch von \textit{Logging} oder \textit{Loggingaktivität} gesprochen.
...
( \textit{not(contains(customerType,'major'))} ) und  der \textit{Link1Copy} die unveränderte. Die Logging-Anweisungen werden bei diesem Ansatz direkt in der flow-Umgebung platziert. 
...
Zuvor muss die Variable .. natürlich deklariert werden. Damit die Variable im ganzen Prozess benutzt werden kann, wird sie als globale Variable (im \textit{variables}-Element des Prozesses) definiert. Zur Vermeidung der Namenskonflikten werden kryptische namen generiert. Da in BPEL durch flow-Aktivität die nebenläufige Ausführung der Aktivitäten möglich ist,    
