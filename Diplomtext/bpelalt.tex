...
von Geschäftsprozessen. BPEL ist 2002 aus IBMs WSFL und Microsofts XLANG entstanden und liegt aktuell in der Version 2.0  als OASIS Standard vor. Durch breite Produktunterstützung hat sich die Sprache mittlerweile auch als Industriestandard etabliert.
...
BPEL-Prozesse können in abstrakte und ausführbare Prozesse unterteilt werden


\begin{itemize}
	\item Abstarkte Prozess Spezifikation der Geschätsprozesse zwischen Partner Beschreibung des nach außen sichtbaren Verhaltens des betrachteten Prozesses die Schnittstellen keine Aussagen über die interne Logik
\end{itemize}
 wobei die ersteren
zur B und die letzteren zur Ausführung auf
einer BPEL-Engine eingesetzt werden. Diese Arbeit beschäftigt sich mit Testen von BPEL-Kompositionen also mit ausführbaren Prozessen. Die abstrakten Prozesse werden aus disem Grund nicht weiter betrachtet.
 ...
 Neben der Beschreibung ausf¨uhrbarer Gesch¨aftsprozesse mittels executable BPEL4WS Prozessen, die
innerhalb einer BPEL4WS Ausf¨uhrungsumgebung ausf¨uhrbar sind, bietet BPEL4WS ebenfalls die
M¨oglichkeit Gesch¨aftsprotokolle (business protocols) zwischen Partnern anhand nicht ausf¨uhrbarer
sog. abstrakter Prozesse zu spezifizieren, die den nach außen sichtbaren Ablauf von zwischen Partnern
existierenden Kollaborationen mittels den zwischen ihnen auszutauschenden Nachrichten charakterisieren.
Jeder abstrakte Prozess beschreibt hierbei das Verhalten eines in einem Gesch¨aftsprotokoll
involvierten Partners aus dessen lokaler Sicht anhand der Spezifikation des Ablaufs von Aktionen zum
Versenden als auch Empfangen von Nachrichten. Somit charakterisieren abstrakte Prozesse das nach
außen sichtbare Verhalten eines Partners (sog. external view), wohingegen ausf¨uhrbare BPEL4WS
Prozesse deren internes Verhalten (sog. internal view) spezifizieren. Zur Beschreibung abstrakter als
auch ausf¨uhrbarer Prozesse kommen in BPEL4WS jeweils die gleichen Konzepte zur Anwendung.

Die BPEL Spezifikation unterscheidet zwischen der Definition abstrakter und der Definition
ausführbarer Prozesse.
Abstrakte Prozesse : Mit der Veröffentlichung abstrakter Prozessbeschreibungen werden
die Schnittstellen eines Prozesses, Nachrichtenformate und die Interaktionsmuster
der angebotenen Operationen bekannt gegeben. Abstrakte Prozesse machen  des beschriebenen Prozesses.
Möchten Unternehmen ihre Prozesse miteinander verknüpfen, so tauschen sie
abstrakte Prozessbeschreibungen aus, dadurch bekommt jedes Unternehmen die
für die Interaktion notwendigen Informationen, verrät aber nicht seine internen
Prozessabläufe.
Ausführbare Prozesse : Ausführbare Prozesse beschreiben alle für die Prozessausführung
notwendigen Informationen (Schnittstellen, Variablen, Nachrichtenformate,
Prozesslogik) auf Schnittstellenebene. Das bedeutet, die Endpunkte der
tatsächlich verwendetenWeb Services sind in der Beschreibung eines ausführbaren
Prozesses noch nicht enthalten. Diese Informationen können zu unterschiedlichen
Zeitpunkten bereitgestellt werden, zum Beispiel bei der Auslieferung des
Prozesses in ein Workflow Management System (siehe dazu Kapitel 5.1.7). Auf
diese Art und Weise kann ein ausführbarer BPEL Prozess unabhängig von seiner
späteren Ausführungsumgebung modelliert werden. Zur Modellierung sind
noch keine existierendenWeb Services notwendig, sondern lediglich die WSDL
Beschreibung der verwendeten Web Services.

ExecutableProcess:
Orchestrierung spezifischer Aktivitäten und bestimmter Dienste, die ausgeführt werden müssen
Über eine ExecutionEngineausführbar
Definition einer Orchestrierung
AbstractProcess:
Spezifikation des Nachrichtenaustauschs mehrerer Teilnehmer
Keine Definition der internen Prozess-Details
Schnittstelle definiert durch Menge aller Receiveund Reply
Definition einer Choreographie

Die mit BPEL erstellten Prozess-Kollaborationen der einzelnen Web Services repräsentieren sich
nach auÿen als eigenständige, übergeordnete Web Services. Nach dem SOA-Prinzip lassen sich somit
schon einmal orchestrierte Geschäftsprozesse zu gröÿeren Geschäftsprozessen zusammenführen.
Dadurch ist auch eine Wiederverwendung der einmal erstellten Dienste gegeben und sie erlaubt damit,
technische Prozesse zu abstrahieren. Die Komplexität der betrachteten Geschäftsprozesse wird
dadurch erst beherrschbar.
Ein weiterer Vorteil von BPEL ist, dass die Änderungen der Geschäftslogik nur in der BPEL-Datei
durchgeführt werden müssen, da eine klare Trennung zwischen der Geschäftslogik und der Implementierung
besteht.
...
Ein BPEL Prozess importiert Schnittstellenbeschreibungen über WSDL [STDWSDL] und stellt seine Schnittstellen als WSDL Dokument zur Verfügung. Der Prozess stellt sich nach außen als Web Service dar, der intern aber wieder-um aus einer Verknüpfung von Web Services sowie Applikationslogik besteht. Somit ist es natürlich möglich, dass ein BPEL Prozess einen anderen BPEL Pro-zess verwendet ohne dies zu bemerken. Die Bindung an einen konkreten Web Service kann hierbei entweder statisch zum Entwicklungs-, oder Deploy-mentzeitpunkt oder dynamisch zur Laufzeit erfolgen.

Die benutzten und angebotenen Schnittstellen werden in BPEL mit Hilfe von WSDL
beschrieben. Die Interaktion eines BPEL Prozesses mit einem Partner entspricht einer
Konversation zwischen zwei Web Services. Eine solche Beziehung zu Partnerdiensten
zeichnet sich durch die Verwendung bestimmter Operationen aus und wird in BPEL
durch PartnerLinks beschrieben. Ein PartnerLink stellt eine Art Konnektor
zwischen zwei Web Services dar, der einen definierten Type besitzt und für jeden der
beteiligten Services festlegt, welchen WSDL PortType dieser für die Interaktion anzubieten
hat.
Für die Typdefinition von PartnerLinks führt BPEL eine WSDL Erweiterung namens
PartnerLinkType ein. Ein PartnerLinkType besteht aus einer oder zwei Rollen.
Jede Rolle ist durch einen bestimmten WSDL PortType beschrieben. Bietet nur
einer der beiden Web Services bestimmte Operationen an, die der andere Web Service
benutzt ohne eine Callbackoperation anbieten zu müssen (One-Way oder synchrone
Interaktion), dann besteht der PartnerLinkType für diese Beziehung nur aus einer
Rolle - der Rolle des Dienstanbieters. Ein PartnerLink legt fest, welche Rollen
die beteiligten Web Services während der Konversation einnehmen.
Abbildung 23 verdeutlicht den beschriebenen Zusammenhang anhand zweier Prozesse,
welche die Auszahlung von Schadenssummen einer Versicherung abbilden.
Der PartnerLinkType zahlstellenPLT definiert eine Beziehung zwischen
zwei Web Services, von denen der eine den PortType zahlstellenCallback-
PT und der andere den PortType zahlstellenPT implementiert. Diese Definition
findet auf einer abstrakten Ebene statt, da nur die WSDL PortTypes
aber keine konkreten WSDL Ports referenziert werden.
2. Der PartnerLink zahlstelle definiert für den Prozess schadenBearbeitung,
dass dieser innerhalb der Konversation die Rolle Schadensbearbeitung
und der Partnerdienst die Rolle Zahlstelle einnimmt.
Der PartnerLink schadensabteilung definiert eine umgekehrte Rollenverteilung
für den Prozess auszahlung.
4. Bei der dargestellten Interaktion handelt es sich um ein asynchrones Interaktionsmuster.
Der Prozess schadensBearbeitung konsumiert die Operation zahle-
Aus des Prozesses auszahlung und bietet zwei Operationen für eine asynchrone
Antwort an.
5. Der Prozess auszahlung verarbeitet die Auszahlungsaufforderung und konsumiert
anschließend zur Bereitstellung des Ergebnisses eine der beiden angebotenen
Operationen auszahlungOK und auszahlungFailed. Eine Auszahlung
kann zum Beispiel fehlschlagen, wenn die Plausibilisierung der Bankleitzahl ein
negatives Ergebnis ergibt.
...
Lebenszyklus von BPEL4WS Prozessen
Da BPEL4WS Prozesse langlebig und zustandsbehaftet und Prozessdefinitionen nur Modelle ausf¨uhrbarer
Gesch¨aftsprozesse sind, werden sie zur Ausf¨uhrungszeit instanziiert.
...
und anhand derer von der BPEL4WS Ausf¨uhrungsumgebung bestimmt, wird ob eine neue Instanz
erzeugt werden soll oder nicht. Die zur Erzeugung von Prozessinstanzen zu verwendeten Aktivit¨aten
werden hierbei anhand des createInstance=true Attributs charakterisiert.
...
Wenn innerhalb des Prozesses asynchrone Interaktionen mit anderen Diensten stattfinden,
werden BPEL Prozesse zu Langläufern, da sie gegebenenfalls sehr lange auf
die Antwort eines Partnerdienstes warten müssen. Immer dann, wenn ein BPEL Prozess
gestartet wird, erstellt das WFMS eine neue Prozessinstanz, welche mit anderen
Partnern interagiert und sich durch einen eigenen Zustand von den restlichen laufenden
Instanzen desselben Prozesstyps unterscheidet. Die durch den Prozess definierten
WSDL Schnittstellen sind dagegen zustandslos.
Nachrichten, die an das WFMS gesendet werden, finden stets im Rahmen einer bestimmten
Konversation zwischen einer Prozessinstanz und einem Partnerdienst statt.
Um die Nachrichten an die korrekte Prozessinstanz weitergeben zu können, muss ein
Identifikationsmechanismus gefunden werden, der es ermöglicht, Anwendungsdaten
auf existierende Prozessinstanzen abzubilden.
BPEL ermöglicht die Identifikation von Prozessinstanzen unter Verwendung der in den
Nachrichten enthaltenen Anwendungsdaten. Die verarbeiteten Daten enthalten häufig
eindeutige Identifikationsmerkmale, wie zum Beispiel die Versicherungsnummer
bei einem Schadensfall. Eine Kombination bestimmter Anwendungsdaten, welche zur
Identifikation von Prozessinstanzen dient, wird als Correlation Set bezeichnet. Pro
Prozess dürfen mehrere Correlation Sets definiert werden, so dass es möglich ist, für
Interaktionen mit unterschiedlichen Geschäftspartnern auch unterschiedliche Anwendungsdaten
zur Korrelation zu verwenden.
Die Erzeugung und Beendigung von Prozessinstanzen geschieht in BPEL implizit.
Beim Eintreffen einer Nachricht für einen bestimmten Prozesstyp, versucht das WFMS
die Nachricht mit Hilfe der enthalten Anwendungsdaten einer existierenden Prozessinstanz
zuzuordnen.Wird keine passende Prozessinstanz gefunden, so erstellt das WFMS
eine neue Instanz des Prozesses und übergibt dieser Prozessinstanz die eingetroffene
Nachricht (siehe Abbildung 24 Nr. 1). Die Aktivität receive markiert den Empfang
von Nachrichten innerhalb des Prozessflusses. Gleichzeitig stellt sie die einzige Möglichkeit
dar, eine neue Prozessinstanz zu erzeugen. Die receive Aktivität besitzt ein
Attribut namen createInstance. Dieses Attribut kann zwei mögliche Werte annehmen
yes und no. Wird dieses Attribut für eine receive Aktivität auf den Wert
yes gesetzt, so signalisiert dies, dass bei einem Nachrichteneingang und nach einem
erfolglosen Zuordnungsversuch eine neue Prozessinstanz erstellt werden darf.

Eine Prozessinstanz wird beendet, wenn die letzte Aktivität des Prozesses beendet ist
(Abb.24 Nr. 2), eine Fehlersituation aufgetreten ist, die nicht innerhalb des Prozesses
behoben werden kann (Abb.24 Nr. 3) oder der Prozess explizit durch die exit
Aktivität beendet wird (Abb.24 Nr. 4).
...
Der BPEL-Standard selbst definiert keine grafische Darstellung der modellierten Prozesse. Dafür ist die BPMN vorgesehen, die im Standard die Abbildung von BPMN nach BPEL beschreibt.
