...
/**
 * The ActivitySection allows the user to add, edit, remove, and move activites of a selected
 * partner track.
 * 
 * @version $Id$
 * @author Philip Mayer
...
public class ActivitySection extends TreeSection {
	...
	private class ActivityLabelProvider implements ILabelProvider {
		...
		public String getText(Object element) {
			if (ActivityUtil.isActivity(element)) {
				...
				return "Condition (" + StringUtils.abbreviate(BPELUnitUtil.removeSpaceLineBreaks( ((XMLCondition) element).getExpression()), 100)
						+ ")";
			}
			return "";
		}
	...
	protected void editPressed() {
		...
		/*
		 * Check for child activities (result of either user selecting such an activity, or a child
		 * element of a child activity).
		 */
		...
		if (currentActivity != null) {
			...
			/*
			 * The wizard pages set their data directly in the XML activities, it is therefore
			 * necessary to copy the activity beforehand, in case the user cancels the wizard. Note
			 * that the wizards need to operate on the original activity, as they are dependent on
			 * navigating the tree structure (for example, for finding the parent track of an
			 * activity).
			 */
			...
			switch (activityConstant) {
				case SEND_ONLY:
					wizard= new SendOnlyWizard(getPage(), ActivityEditMode.EDIT, (XMLSendActivity) currentActivity);
					break;
				case RECEIVE_ONLY:
					wizard= new ReceiveOnlyWizard(getPage(), ActivityEditMode.EDIT, (XMLReceiveActivity) currentActivity);
					break;
				case SEND_RECEIVE_SYNC:
					wizard= new SendReceiveSyncActivityWizard(getPage(), ActivityEditMode.EDIT, (XMLTwoWayActivity) currentActivity);
					break;
				case RECEIVE_SEND_SYNC:
					wizard= new ReceiveSendSyncActivityWizard(getPage(), ActivityEditMode.EDIT, (XMLTwoWayActivity) currentActivity);
					break;
				case SEND_RECEIVE_ASYNC:
					wizard= new SendReceiveAsyncActivityWizard(getPage(), ActivityEditMode.EDIT, (XMLTwoWayActivity) currentActivity);
					break;
				case RECEIVE_SEND_ASYNC:
					wizard= new ReceiveSendAsyncActivityWizard(getPage(), ActivityEditMode.EDIT, (XMLTwoWayActivity) currentActivity);
					break;
			}
			if (wizard != null) {
				if (code != null)
				...
				if (openWizard(wizard)) {
					...
					replaceActivity(fCurrentPartnerTrack, currentActivityIndex, activityConstant, copiedActivity);
	...
	private void replaceActivity(XMLTrack track, int index, ActivityConstant type, XMLActivity theReplacement) {
		if (index != -1) {
			switch (type) {
				case SEND_ONLY:
					...
					break;
				case RECEIVE_ONLY:
					fCurrentPartnerTrack.setReceiveOnlyArray(index, (XMLReceiveActivity) theReplacement);
					break;
				case SEND_RECEIVE_SYNC:
				...
				case RECEIVE_SEND_SYNC:
					...
					break;
				case SEND_RECEIVE_ASYNC:
					fCurrentPartnerTrack.setSendReceiveAsynchronousArray(index, (XMLTwoWayActivity) theReplacement);
					break;
				case RECEIVE_SEND_ASYNC:
					fCurrentPartnerTrack.setReceiveSendAsynchronousArray(index, (XMLTwoWayActivity) theReplacement);
					break;
			}
		}
	}
	...
	protected void addPressed() {
		...
		FieldBasedInputDialog dialog= new FieldBasedInputDialog(getShell(), "Create a new activity");
		...
		if (textData != null) {
			...
			try {
				...
				if (cursor.toFirstChild()) {
					...
					XMLActivity newOne= ActivityUtil.createNewTopLevelActivity(fCurrentPartnerTrack, type);
					newOne.set(obj);
	...
	protected void fillContextMenu(IMenuManager manager) {
		...
		if ( (ssel.size() == 0)
				|| ( (ssel.size() == 1) && ActivityUtil.isActivity(ssel.getFirstElement()) && !ActivityUtil.isChildActivity(ssel.getFirstElement()))) {
		...
		if (ssel.size() == 1) {
			...
			if (ActivityUtil.isActivity(ssel.getFirstElement()) && !ActivityUtil.isChildActivity(ssel.getFirstElement())) {
	...
	protected void addActivity(ActivityConstant constant) {
		...
		switch (constant) {
			...
			case SEND_RECEIVE_SYNC: {
				...
				// Open the wizard
				SendReceiveSyncActivityWizard wiz= new SendReceiveSyncActivityWizard(getPage(), ActivityEditMode.ADD, sendRcvOp);
				if (!openWizard(wiz)) {
			...
			case RECEIVE_SEND_SYNC: {
				...
				// Open the wizard
				ReceiveSendSyncActivityWizard wiz= new ReceiveSendSyncActivityWizard(getPage(), ActivityEditMode.ADD, rcvSendOp);
				if (!openWizard(wiz)) {
			...
			case SEND_RECEIVE_ASYNC: {
				...
				// Open the wizard
				SendReceiveAsyncActivityWizard wiz= new SendReceiveAsyncActivityWizard(getPage(), ActivityEditMode.ADD, sendRcvOp);
				if (!openWizard(wiz)) {
					removeActivity(fCurrentPartnerTrack, sendRcvOp);
				} else
					added= sendRcvOp;
			...
			case RECEIVE_SEND_ASYNC: {
				...
				// Open the wizard
				ReceiveSendAsyncActivityWizard wiz= new ReceiveSendAsyncActivityWizard(getPage(), ActivityEditMode.ADD, sendRcvOp);
				if (!openWizard(wiz)) {
					removeActivity(fCurrentPartnerTrack, sendRcvOp);
				} else
					added= sendRcvOp;
