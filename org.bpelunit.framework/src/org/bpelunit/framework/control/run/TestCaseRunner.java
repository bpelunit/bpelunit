...
/**
 * 
 * The TestCaseRunner handles kicking off the partnerTrack threads; it handles communication between
 * threads as well as network communication and aborting in case of an error, failure, or user
 * interaction.
 * 
 * @version $Id$
 * @author Philip Mayer
...
public class TestCaseRunner {
	...
	public void run() {
		...
		if (fProblemOccurred || fAbortedByUser) {
			...
			/*
			 * HTTPClient methods are not impressed by interruptions. Deal with them the "hard way":
			 * Calling this method effectively closes the I/O connections beneath the HTTP POSTs.
			 */
	 ...
	 * 
	 * This call will not return until either a message as been received, a timeout has occurred, or
	 * the thread was interrupted.
	 * 
	 * Note that the Web Service Handler expects an answer to every such incoming message. Use
	...
	/**
	 * 
	 * Sends back a message as an answer to a previously-received incoming message via
	 * {@link #receiveMessage(PartnerTrack)}. The message will be sent back using the still-open
	 * HTTP Connection.
	 * 
	 * This call will not return until either the message has been delivered, or a timeout has
	...
	public IncomingMessage sendMessageSynchronous(OutgoingMessage message) throws SynchronousSendException, InterruptedException {
		...
		method.getParams().setParameter(HttpMethodParams.SO_TIMEOUT, new Integer(BPELUnitRunner.getTimeout()));

		method.addRequestHeader("SOAPAction", "\"" + message.getSOAPHTTPAction() + "\"");
		RequestEntity entity;
		try {
			entity= new StringRequestEntity(message.getBody(), BPELUnitConstants.TEXT_XML_CONTENT_TYPE, BPELUnitConstants.DEFAULT_HTTP_CHARSET);
	...
	/**
	 * This method signals that the given partner track has completed its actions without any fault.
	 * The partner track thread should terminate directly after calling this method.
	 * 
	 * @param track
	 */
	...
	/**
	 * This method signals that the given partner track has completed its actions with an error or
	 * fault. The partner track thread should terminate directly after calling this method.
	 * 
	 * @param track
	 */
	...
	public void abortTest() {
		/*
		 * Set aborted flag. Will get picked up by the runner in the right thread.
		 */
		fAbortedByUser= true;
	}
