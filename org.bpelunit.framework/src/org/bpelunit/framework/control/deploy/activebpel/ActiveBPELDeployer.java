...
public class ActiveBPELDeployer implements IBPELDeployer {
	...
	public void deploy(String pathToTest, ProcessUnderTest put) throws DeploymentException {
		...
		if (!uploadingFile.exists())
			throw new DeploymentException("ActiveBPEL deployer could not find BPR file " + fBPRFile);

		File resultingFile= new File(fDeploymentDirectory, FilenameUtils.getName(fBPRFile));
		...
		try {
			...
			throw new DeploymentException("An input/output error occured in ActivBPEL deployer when deploying: " + e.getMessage());
		} catch (SOAPException e) {
			throw new DeploymentException("An error occurred while creating SOAP message for ActiveBPEL deployment: " + e.getMessage());
		}
		...
		try {
			...
			if (statusCode < 200 || statusCode > 299) {
				...
				throw new DeploymentException("ActiveBPEL Server reported a Deployment Error: " + responseBody);
			}
